...
import iconicBg from "../assets/hero_background_1.jpg";
...
const HeroSection = ({ configData, landingPageData, handleOrderNow }) => {
  ...
  const ismall = useMediaQuery(theme.breakpoints.down("sm"));
      ...
      >
        <Box sx={{ position: "absolute" }} className="shape">
          <CustomImageContainer
            src={iconicBg.src}
            alt={t("Background")}
            height={ismall ? "auto" : isTab ? "28rem" : "31rem"}
            width="100%"
            borderRadius="20px"
            objectFit="cover"
          />
        ...
        <Grid container>
          <Grid
            ...
            xs={8}
            md={7}
            sx={{ padding: { xs: "1rem", sm: "2rem", md: "7rem 3rem 0rem 8rem" } }}
          >
            <NoSsr>
              <HeroTitleSection
